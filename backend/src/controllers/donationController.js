...
export async function createDonation(req, res) {
  try {
    ...
    if (!campaign_id || !amount) return res.status(400).json({ error: "campaign_id and amount are required" });
    ...
    const { data, error } = await supabase.rpc("add_donation_and_increment", params);
    ...
    if (donationId && typeof donationId === "object" && Object.keys(donationId).length === 1) {
...
export async function webhookHandler(req, res) {
  try {
    ...
    const { provider, provider_payment_id, status, amount, currency = "INR", campaign_id, donor_name = null, donor_email = null, metadata = {} } = req.body;
    if (!provider || !provider_payment_id || !status) return res.status(400).json({ error: "provider, provider_payment_id and status required" });

    // Try find existing donation by provider_payment_id
    const { data: existing, error: fetchErr } = await supabase.from("donations").select("*").eq("provider_payment_id", provider_payment_id).maybeSingle();
    if (fetchErr) { console.error("webhook fetch err", fetchErr); return res.status(500).json({ error: "DB error" }); }

    if (existing) {
      // If already in desired status -> idempotent OK
      if (existing.status === status) return res.json({ ok: true, donation_id: existing.id });
      ...
      if (updateErr) { console.error("webhook update err", updateErr); return res.status(500).json({ error: "Failed to update donation" }); }

      // Adjust campaign current_amount if needed
      if (status === "succeeded" && existing.status !== "succeeded") {
        await supabase.rpc("increment_campaign_amount", { p_campaign: existing.campaign_id, p_amount: existing.amount });
    ...
    if (!campaign_id || !amount) return res.status(400).json({ error: "campaign_id and amount required for new donation" });

    const { data: createdId, error: rpcErr } = await supabase.rpc("add_donation_and_increment", {
      p_campaign: campaign_id,
      p_donor: null,
      p_donor_name: donor_name,
      p_donor_email: donor_email,
    ...
    if (rpcErr) { console.error("webhook rpc err", rpcErr); return res.status(500).json({ error: "Failed to create donation" }); }
